...
import { HEXES_PER_INITIAL_MOB, HEXES_PER_ITEM, HEXES_PER_MAX_MOB } from './constants';
...
function initGame(selectedRadius: number): void {
  ...
  // --- ステップ2: 計算済みの値を使って、完全なGameStateオブジェクトを作成 ---
  const initialState: GameState = {
    ...
    nextTrapId: 0,
    isPlacingTrap: null,
    isDangerVisible: dangerToggle.checked, // 現在のUIの状態を引き継ぐ
  ...
  // --- ステップ3: 完成したオブジェクトをグローバルな状態として設定 ---
